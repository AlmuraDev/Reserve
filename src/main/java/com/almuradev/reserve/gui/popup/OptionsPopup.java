...
public class OptionsPopup extends GenericPopup {
	private final ReservePlugin plugin;
	private final SpoutPlayer sPlayer;
	Color bottom = new Color(1.0F, 1.0F, 1.0F, 0.50F);

	public OptionsPopup(ReservePlugin plugin, SpoutPlayer sPlayer) {
		this.plugin = plugin;
	...
	public void onClickCommand(int commandGoal) {
		switch (commandGoal) {
			case 1:
				sPlayer.getMainScreen().closePopup();
				new ReserveConfigPopup(plugin, sPlayer, null);
				break;
			case 2:
				sPlayer.getMainScreen().closePopup();
				new BankConfigPopup(plugin, sPlayer, null);
				break;
			case 3:
				sPlayer.getMainScreen().closePopup();
