...
public class BankConfigPopup extends GenericPopup {
	private final ReservePlugin plugin;
	private final SpoutPlayer sPlayer;
	private final Bank selectedBank;
	private final Color bottom = new Color(1.0F, 1.0F, 1.0F, 0.50F);

	public BankConfigPopup(ReservePlugin plugin, SpoutPlayer sPlayer, Bank bank) {
		this.plugin = plugin;
	...
	public void onClickCommand(int commandGoal) {
		switch (commandGoal) {
			case 1:
				sPlayer.getMainScreen().closePopup();
				new AckPopup(plugin, sPlayer, selectedBank, "Bank Configuration Saved", "bankconfiggui");
				break;
			case 2:
				sPlayer.getMainScreen().closePopup();
